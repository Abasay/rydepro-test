...
const ChangePin = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      setSettings({
        ...settings,
        setHeadingText: 'Confirm your 4-digit pin',
        setSubHeadingText: '',
        securityOptions: {
  ...
  const handleChange = (
    ...
    formik.setFieldValue(`pin[${idx}]`, value);
  ...
  useEffect(() => {
    setSettings({ ...settings, setHeadingText: 'Change Pin' });
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className='flex flex-col gap-[24px] md:mt-6 mt-0'>
        ...
        <div className='flex justify-center md:relative gap-[24px] px-[20px]'>
          {formik.values.pin.map((_, idx) => (
            <input
              key={idx}
              ref={(el: any) => (inputsRef.current[idx] = el)}
              className='focus-within:outline-none h-[56px] md:w-[56px] text-[#111111] placeholder:text-[#111111] rounded-[4px] text-[20px] border-y-[1px] md:border-b-[1px] md:border-t-0 bg-transparent border-[#8A8A8A] gap-[8px] text-center font-[500]'
              maxLength={1}
              type='text'
              inputMode='numeric'
              pattern='[0-9]*'
              placeholder='*'
              value={formik.values.pin[idx]}
