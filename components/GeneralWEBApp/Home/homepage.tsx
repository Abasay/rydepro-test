...
import ellipses from '@/components/GeneralWEBApp/Home/svgs/ellipses.svg';
import faceID from '@/components/GeneralWEBApp/Home/svgs/faceID.svg';
import fingerID from '@/components/GeneralWEBApp/Home/svgs/fingerID.svg';
import { homepageFooter } from '@/components/GeneralWEBApp/Home/data/data';
...
const Homepage = () => {
  ...
  const {
    setIsHomePageActive: setLoginIsHomePageActive,
    signIn,
    setSignIn,
  } = useGeneralLogInContext();
  const {
    reset,
    setReset,
    setIsHomePageActive: setResetHomePageActive,
  } = useResetContext();
  ...
  const handleNavigation = (text: string) => {
    if (
      (pathname.includes('/login') && text === 'Sign Up') ||
      (pathname.includes('/reset') && text === 'Sign Up')
    ) {
      window.location.href = '/general/signup';
      return;
    }
    if (
      (pathname.includes('/login') && text === 'Account Recovery') ||
      (pathname.includes('/signup') && text === 'Account Recovery')
    ) {
      window.location.href = '/general/reset';
      return;
    }
    {
      text === 'Sign Up' &&
  ...
  return (
    <GeneralDesign>
      ...
      <main
        className={`container h-screen flex flex-col justify-center items-center ${styles['slide-from-bottom']}`}
      >
        <section className='w-full md:w-[614px] h-[550px] mt-10 md:mt-0 rounded-[24px] py-[48px] md:p-[48px] flex flex-col gap-[80px] md:bg-[#FFFFFF] md:shadow-sm'>
          <div className='head w-full md:w-[518px] h-[60px] gap-[4px] hidden md:flex md:flex-col'>
            ...
            <span className='font-normal text-base leading-[24px] text-[#3C3C3C]'>
              Sign In Now!
            </span>
          </div>
          <div className='flex flex-col gap-[48px] justify-center h-[184px] items-center'>
            <div className='w-[240px] h-[112px] flex gap-[80px]'>
              <div
                title='Finger ID'
                className='flex flex-col w-[80px] h-[80px] gap-[8px] cursor-pointer'
              >
                <Image src={fingerID} width={80} height={80} alt='' />
                <h2 className='text-base leading-[24px] text-center font-bold text-[#0E0E0E]'>
                  Finger ID
                </h2>
              </div>
              <div
            ...
            <span
              onClick={() => {
                // if (pathname.includes('/login')) {
                //   setLoginIsHomePageActive(false);
                //   setSignIn({ ...signIn, isSignInPageActive: true });
                //   return;
